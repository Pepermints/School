function [ weights_history_e, error_history ] = run(N_cycles, x, alpha_e, init_w_e, N_input_e);
...
function [ activity ] = getInputActivity(position, tuningcurve, N_input);
    ...
    for i = 1:N_input
        activity(i) = gaussmf(position, [0.005 tuningcurve(i)])*15;    
    end
end
...
function [centers] = getTuningCurves(x, N_input);
    ...
    centers = 15 + (0:1:(N_input-1))*(22-15)/(N_input-1);
end
